 ...
 */
public class Q1 {
    private static int valueMaxCount = 20;
    ...
    public static void initData() {
        ...
        System.out.println(Arrays.toString(values).concat(" ---原始数据"));
    ...
    public static void scheme1() {
        ...
        System.out.println(Arrays.toString(valueIndexes).concat(" ---将数据放在坐标上"));
        for (int i = 0; i < valueIndexes.length; i++) {
            if (valueIndexes[i] == 1)
                System.out.print(String.valueOf(i).concat(", "));
        }
        System.out.println(" ---所有存在值的坐标点（即所有的值）");
    }
}
